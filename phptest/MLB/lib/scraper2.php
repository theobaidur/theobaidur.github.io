...
class Scraper{
  ...
  public function save_csv_and_json($slug, $data, $fix_required=false){
    $file_datestamp = (date("m-d-y_g-i-s-A"));
	#if fix requred, save them to predata folder
	if($fix_required){
		// csv
		...
		//if fix not requred, save them to finaldata folder and copy them to dev & prod; as json
		
		// to finaldata
	    $output = dirname(__FILE__) . '/../finaldata/live/' . $slug . '.json';
	    if(file_exists($output)) copy($output, dirname(__FILE__) . '/../finaldata/archive/' . $slug . '_'.$file_datestamp . '.json');
	    file_put_contents($output, json_encode($data));
		
		// to dev
		$dev_output = '/home/bpender/webapps/dev/data/' . $slug . '.json';
		file_put_contents($dev_output, json_encode($data));
		
		// to prod
		$prod_output = '/home/bpender/webapps/lineuplab/data/' . $slug . '.json';
		file_put_contents($prod_output, json_encode($data));
