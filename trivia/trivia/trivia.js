...
function Trivia() {
	//
	var obth = this;
	// stores the object contained in 'this'
	var objfiles = {};
	// stores the object with "category":"file_name" elements, received in setCateg()
	var level = 'level1';
	// 'level1' with variant of answers, 'level2' with text field to write the answer
	var stopct = 0;
	// if not 0, countdown-timer not continue, in startCT()
	var ctsec = 15;
	// number of seconds for each quiz
	var nquizzes = 0;
	// total number of quizzes
	var nquiz = 0;
	// stores number of current quiz
	var canswer = '';
	// for correct answer of current quiz
	...
	var qmode = 'qindex';
	var qctimer = 0;
	// for countdown-timer (if is 1), shown in #sctimer
	var iquizzes = [];
	// array with indexes of quizzes that must to get randomly
	...
	this.setCateg = function(objf, type) {
		...
		var i = 0;
		// to can call the method that sets the quizzes, with the first file_name
		...
		if (document.getElementById('tcateg'))
			document.getElementById('tcateg').innerHTML = 'Trivia Category: <select ' + onchg + '>' + tcateg + '</select>';
	}
	...
	this.setQuizXML = function(propf) {
		...
		xmlhttp.open("GET", 'trivia/' + objfiles[propf], false);
		xmlhttp.send(null);
		var xml_dom = xmlhttp.responseXML;
		// gets the XML DOM from xml file

		var arr_quiz = xml_dom.getElementsByTagName('quiz');
		// gets an Array with all "quiz" tags
		nquizzes = arr_quiz.length;
		// number of 'quiz' elements (total quizzes)

		// traverses the "arr_quiz" array
		for (var i = 0; i < nquizzes; i++) {
			quizzes[i] = {};
			// creates each element in quizzes array, an object that will conteins question elements

			// adds in quizzes each 'quiz' element with question (<que>), correct answer(<ca>), and incorrect answers(<ia>)
			quizzes[i]['que'] = arr_quiz[i].getElementsByTagName('que')[0].childNodes[0].nodeValue;
			// question
			quizzes[i]['ca'] = arr_quiz[i].getElementsByTagName('ca')[0].childNodes[0].nodeValue;
			// correct answer

			// sets an array with incorrect answers
			quizzes[i]['ia'] = [];
			var arr_ia = arr_quiz[i].getElementsByTagName('ia');
			var nr_ia = arr_ia.length;
			// number of 'ia' elements
		...
		if (document.getElementById('totalq'))
			document.getElementById('totalq').innerHTML = nquizzes;
		if (document.getElementById('ntotalq'))
			document.getElementById('ntotalq').innerHTML = nquizzes;
	}
	...
	this.setQuizJSON = function(propf) {
		...
		// make the request to get the file content
		xmlhttp.open("GET", 'trivia/' + objfiles[propf], true);
		xmlhttp.send(null);

		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4) {
				var tobj = xmlhttp.responseText;
				// puts in a variable the received data
				tobj = tobj.replace(/\\|\r\n|\n|\r/gm, '');
				// remove slashes "\" and new lines
				eval("quizzes =" + tobj);
				// adds the JSON object in quizzes property
	...
	var startCT = function() {
		...
		if (ctsec >= 0) {
			...
			if (stopct == 0 && document.getElementById('sctimer')) {
				...
				$("#counter_con").text(ctsec);
				setTimeout(startCT, 1000);
				// Auto-calls the function after 1 second
			}
			ctsec--;
			// decreases the value with one unit
		} else
			obth.getAnswer(0);
	...
	var setQmode = function() {
		if (document.getElementById('qindex').checked == true)
			qmode = 'qindex';
		else if (document.getElementById('qrandom').checked == true)
			qmode = 'qrandom';
	}
	...
	var setIQrandom = function(stn) {
		...
		if (stn == 'start') {
			...
			var nqr = iquizzes.length;
			// number of quizzes index in iquizzes
			for (var i = 0; i < nqr; i++) {
				if (iquizzes[i] == nquiz) {
					iquizzes.splice(i, 1);
					break;
	...
	var setNquiz = function(stn) {
		...
		if (qmode == 'qrandom')
			...
			if (nquiz < (nquizzes - 1))
				...
				obth.resetTrivia();
			// if reached the last quiz
		}
	}
	// starts and shows the quiz, receives the way: 'start' or 'next' to set the index of current quiz
	this.sQuiz = function(stn) {
		$("#qansw").removeClass("result");
		setQmode();
		// to set the game mode
		setNquiz(stn);
		// to set the index of current quiz
		deBtns(['level1', 'level2', 'qindex', 'nquiz', 'qrandom', 'qctimer', 'squiz'], 'disable');
		// to disable buttons (enable Reset)
		canswer = quizzes[nquiz]['ca'];
		...
		if (document.getElementById('quiz')) {
			...
			document.getElementById('quiz').innerHTML = addct + '<h4 id="qnr">Quiz No. ' + (nquiz + 1) + '</h4><h4 id="tquestion">' + quizzes[nquiz]['que'] + '</h4>';
			document.getElementById('options').innerHTML = '<div id="qansw">' + (level == 'level1' ? qAnswerL1() : qAnswerL2()) + '</div>';
	...
	this.resetTrivia = function() {
		nquiz = 0;
		// index-number of quiz
		qctimer = 0;
		// to disable countdown-timer
		...
		deBtns(['level1', 'level2', 'qindex', 'nquiz', 'qrandom', 'qctimer', 'squiz'], 'enable');
		// to enable buttons (disable Reset)
		document.getElementById('nquiz').value = 1;
		document.getElementById('qctimer').checked = false;
		document.getElementById('quiz').innerHTML = '<h3>Trivia Challenge</h3><strong>Levels:</strong><ul><li>Level 1 - Easy - Displays a list of answers to each quiz.</li><li>Level 2 - Difficult - Displays a text box in which you write the answer.</li></ul><strong>Modes</strong><ul><li>Consecutive - Questions start from the quiz by index number added into a text field, then added in their order till the last quiz.</li><li>Random - The quizzesQuestions are choosen randomly, untill the last quiz, without repeating.</li></ul>&bull;<strong>Countdown Timer</strong> - If checked, you have 15 seconds to answer untill the next quiz is added automatically.<br/><br/> - Click the Start button to start the quizzes. The Reset button resets the Quiz.<div id="tfrom">From: <a href="http://poststreet.com/trivia/" title="Trivia Quiz Challenge">http://poststreet.com/trivia</a></div>';
	}
	...
	var deBtns = function(ids, de) {
		...
		if (de == 'disable')
			document.getElementById('treset').removeAttribute('disabled');
		else
	...
	var qAnswerL1 = function() {
		var qhtml = '';
		// quiz question
		var nr_ia = quizzes[nquiz]['ia'].length;
		// number of incorrect answers
		...
		var serial_count = 0;
		var serial_letter = ['A','B','C','D','E','F','G','H','I','J'];

		// creates the options with answers
		for (var i = 0; i < nr_ia; i++,serial_count++) {
			// if 'i' = randomnr, adds correct answer, and decrease the index, else, incorrect answer
			if (i == randomnr) {
				qhtml += '<div class="qanswer" onclick="obTrivia.getAnswer(this)">'+serial_letter[serial_count]+'. ' + canswer + '</div>';
				randomnr = -1;
				// to not be again equal to i
				i--;
			} else
				qhtml += '<div class="qanswer" onclick="obTrivia.getAnswer(this)">'+serial_letter[serial_count]+'. ' + quizzes[nquiz]['ia'][i] + '</div>';
		}
	...
	var answered = function() {
		if (document.getElementById('ntotalq'))
			document.getElementById('ntotalq').innerHTML = nquizzes;
		if (document.getElementById('nqansw'))
			document.getElementById('nqansw').innerHTML = nqansw;
		if (document.getElementById('nca'))
			document.getElementById('nca').innerHTML = nca;
		if (document.getElementById('nia'))
			document.getElementById('nia').innerHTML = nia;
	}
	// method called when an answer is clicked, receives the object with answer
	this.getAnswer = function(answ) {
		var status = $('<div>').attr('id', 'round_status');
		...
		if (document.getElementById('qansw')) {
			...
			if (answ != 0)
				var qanswer = answ.id == 'qanswerl2' ? answ.value.replace(/^\s+|\<+|\>+|\s+$/g, '') : answ.innerHTML.replace(/^\s+|\s+$/g, '');
		...
		status.html($("#answered").html());

		$("#qansw").prepend(status);

		$("#qansw").addClass("result");
	...
	if (document.getElementById('level1'))
		document.getElementById('level1').onclick = function() {
			obth.setLevel('level1');
		}
	if (document.getElementById('level2'))
		document.getElementById('level2').onclick = function() {
			obth.setLevel('level2');
		}
	// onclick on Radio buttons for mode, to show /hide the field for starting quiz number
	if (document.getElementById('qindex'))
		document.getElementById('qindex').onclick = function() {
			document.getElementById('startqn').style.display = 'block';
		}
	if (document.getElementById('qrandom'))
		document.getElementById('qrandom').onclick = function() {
			document.getElementById('startqn').style.display = 'none';
		}
	// onclick on Checkbox button for countdown-timer
	if (document.getElementById('qctimer'))
		document.getElementById('qctimer').onclick = function() {
			obth.setQctimer(this);
		}
	// onclick on Start button
	if (document.getElementById('squiz'))
		document.getElementById('squiz').onclick = function() {
			obth.sQuiz('start');
			$("#wrapper").addClass('state_running');
			var cat = $("#tcateg option:selected").text();
			$("#category_con").text(cat);
			$("#tcateg").hide();
		}
...
var obTrivia = new Trivia();
// object of the Trivia class
