...
qq.Templating = function(spec) {
    ...
    qq.extend(this, {
        ...
        renderComplete:function(){
            /*My custom code*/
            var bin = document.getElementById('scrollUploadedImages');

            bin.addEventListener('dragover', function (e) {
                if (e.preventDefault) e.preventDefault(); // allows us to drop
                if($(bin).find(".tr_classstyle").length<=10 && $(bin).find(".upload_in_progress").length==0){
                    $(this).find(".qq-upload-drop-area").addClass("qq-upload-drop-area-active");
                    $(bin).scrollTop(0);
