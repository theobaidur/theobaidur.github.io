(function($){
	$.fn.FillSocialInfo = function(options){
		...
		var fetch_json = function(word){
			...
			currentQuery = $.ajax({
			    type: "GET",
			    url: "http://socialsearch.club/json-out.php",
			    data: "query="+word,
		...
		var processResult = function(result){
			var ul = outerWrapper.find(".socialInfoSuggestion");
			ul.empty();
		...
		theWrapper.keyup(function(e){
			...
			if(charCount>3){
				if(outerWrapper.find(".socialInfoSuggestion").length){
					outerWrapper.find(".socialInfoSuggestion").css(getCaretPixelPos(theWrapper)).empty().html("<li class='socailInfoLink'>Loading...</li>");
				}else{
					$("<ul>").addClass("socialInfoSuggestion")
					.mouseenter(function(){
						focusOnList = true;
					})
					.mouseleave(function(){
					...
					.css(getCaretPixelPos(theWrapper))
					.html("<li class='socailInfoLink'>Loading...</li>")
